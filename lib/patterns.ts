...
export function getPatternById(id: string): Pattern | undefined {
  return gridPatterns.find((p) => p.id === id)
}

// Get random pattern
export function getRandomPattern(): Pattern {
  const randomIndex = Math.floor(Math.random() * gridPatterns.length)
  return gridPatterns[randomIndex]
}

// Get patterns by category
export function getPatternsByCategory(categoryId: CategoryId): Pattern[] {
  if (categoryId === "all") return gridPatterns
  return gridPatterns.filter((p) => p.category === categoryId)
}
